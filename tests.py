...
from stereovision import stereo_cameras


# # Global variables preset
imageToDispL = './scene_l.jpg'   # Left photo path
imageToDispR = './scene_r.jpg'   # Right photo path
#
# if not (os.path.isfile(imageToDispL) and os.path.isfile(imageToDispR)):
#     print('Can not read image from file!')
#     exit(0)

# imgLeft, imgRight = stereo_cameras.StereoPair([0, 2]).get_frames()
# Read image from photo
imgLeft = cv2.imread(imageToDispL, 0)
imgRight = cv2.imread(imageToDispR, 0)


# # Implementing calibration data
# print('Read calibration data and rectifying stereo pair...')
# calibration = StereoCalibration(input_folder='calib_result')     # Здесь тебе надо подгрузить свою калибровку
# rectified_pair = calibration.rectify((imgLeft, imgRight))        # возможно она у тебя немного в другом формате, но я думаю, что
#                                                                  # тоже формат пригодный для модуля StereoCalibration
...
plt.subplot(1, 2, 1)
dmObject = plt.imshow(rectified_pair[1], 'gray')
